...
BEGIN
  ...
  IF pricing.user_id IS NULL THEN
    ...
    pricing.effective_price := public.calc_pro_price(pricing.active_direct_referrals);
  END IF;
  ...
  SELECT (value #>> '{}')::numeric INTO base_price FROM public.referral_config WHERE key = 'pro_base_price';
  ...
  SELECT (value #>> '{}')::integer INTO max_referrals FROM public.referral_config WHERE key = 'pro_max_discount_referrals';

  next_threshold := LEAST(
    (FLOOR(pricing.active_direct_referrals::numeric / per_step) + 1) * per_step,
    max_referrals
  );
  next_price := public.calc_pro_price(next_threshold);

  RETURN jsonb_build_object(
    'referral_code', code,
    'active_direct_referrals', pricing.active_direct_referrals,
    'current_price', pricing.effective_price,
...
AS $$
  SELECT jsonb_build_object(
    ...
    'users_with_pro_discount', (SELECT count(*) FROM public.pro_referral_pricing WHERE effective_price < (SELECT (value #>> '{}')::numeric FROM public.referral_config WHERE key = 'pro_base_price')),
