...
import { type BioInput, type BioOutput, formatBioForExport, generateBioLocal } from "@/components/tools/bio/bioLogic";
...
export function BioTool({ runtime }: { runtime: ToolRuntime }) {
  ...
  function downloadOutput() {
    if (!output) return;
    const blob = new Blob([formatBioForExport({ ...input, tone }, output)], { type: "text/plain;charset=utf-8" });
      ...
      <div className="panel space-y-5 p-6">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Bio per Amazon Author Central, sito e social, più comunicato stampa per il lancio del libro.
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Input id="bio-name" value={input.authorName} onChange={(e) => update({ authorName: e.target.value })} placeholder="Es. Marco Bianchi" />
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input id="bio-book" value={input.bookTitle} onChange={(e) => update({ bookTitle: e.target.value })} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="bio-release">Data / info uscita (opzionale)</Label>
            <Input id="bio-release" value={input.releaseInfo} onChange={(e) => update({ releaseInfo: e.target.value })} placeholder="Es. 15 ottobre 2026" />
          </div>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="bio-links">Sito / social da citare (opzionale)</Label>
          <Input id="bio-links" value={input.links} onChange={(e) => update({ links: e.target.value })} placeholder="Es. www.marcobianchi.it — @marcobianchi.autore" />
        </div>
      ...
      <div className="panel space-y-4 p-6">
        ...
        {!output && (
          <p className="text-sm text-muted-foreground">
            Compila il form e premi "Genera" per ottenere 3 bio di lunghezza crescente e un comunicato stampa pronto.
          </p>
        )}
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                ...
                <Button variant="ghost" size="sm" onClick={() => copyToClipboard(output.shortBio, "Bio breve")}>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                ...
                <Button variant="ghost" size="sm" onClick={() => copyToClipboard(output.mediumBio, "Bio media")}>
                  <Copy className="size-4" />
                </Button>
              </div>
              <Textarea readOnly rows={4} className="text-sm leading-relaxed" value={output.mediumBio} />
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                ...
                <Button variant="ghost" size="sm" onClick={() => copyToClipboard(output.longBio, "Bio lunga")}>
                  <Copy className="size-4" />
                </Button>
              </div>
              <Textarea readOnly rows={6} className="text-sm leading-relaxed" value={output.longBio} />
            </div>

            <div className="space-y-1.5">
              <div className="flex items-center justify-between gap-3">
                <Label className="text-xs tracking-wide uppercase text-muted-foreground">Comunicato stampa</Label>
                <Button variant="ghost" size="sm" onClick={() => copyToClipboard(output.pressRelease, "Comunicato stampa")}>
                  <Copy className="size-4" />
                </Button>
              </div>
              <Textarea readOnly rows={10} className="font-mono text-xs leading-relaxed" value={output.pressRelease} />
            </div>
